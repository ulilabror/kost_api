...
class Auth extends CI_Controller
{
	...
	public function edit_user($id)
	{
		...
		$this->form_validation->set_rules('no_wa', $this->lang->line('edit_user_validation_nowa_label'), 'trim|required');
		$this->form_validation->set_rules('alamat', $this->lang->line('edit_user_validation_alamat_label'), 'trim|required');
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			if ($this->form_validation->run() === TRUE) {
				...
				if (!$this->upload->do_upload('userfile')) {
					...
					unlink(realpath(APPPATH . '../assets/img/avatars/' . $user->avatar));
		...
		$this->data['no_wa'] = [
			...
			'id'    => 'no_wa',
			'type'  => 'text',
			'value' => $this->form_validation->set_value('no_wa', $user->no_wa),
		];
		$this->data['email'] = [
			'name' => 'email',
			'class' => 'form-control form-control-user',
